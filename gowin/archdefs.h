...
struct DelayInfo
{
    delay_t min_raise_delay = 0, max_raise_delay = 0;
    delay_t min_fall_delay = 0, max_fall_delay = 0;

    delay_t minRaiseDelay() const { return min_raise_delay; }
    delay_t maxRaiseDelay() const { return max_raise_delay; }

    delay_t minFallDelay() const { return min_fall_delay; }
    delay_t maxFallDelay() const { return max_fall_delay; }

    delay_t minDelay() const { return std::min(min_raise_delay, min_fall_delay); }
    delay_t maxDelay() const { return std::max(max_raise_delay, max_fall_delay); }

    DelayInfo operator+(const DelayInfo &other) const
    {
        DelayInfo ret;
        ret.min_fall_delay = this->min_fall_delay + other.min_fall_delay;
        ret.max_fall_delay = this->max_fall_delay + other.max_fall_delay;
        ret.min_raise_delay = this->min_raise_delay + other.min_raise_delay;
        ret.max_raise_delay = this->max_raise_delay + other.max_raise_delay;
        return ret;
    }
};
